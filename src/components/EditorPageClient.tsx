...
type TabKey = 'mint' | 'editor' | 'stats' | 'about';
...
export default function EditorPageClient() {
  ...
  async function handleMintSuperInu() {
    try {
      // TODO: point this to your mint frame/route
      await sdk.actions.openMiniApp({
        url: 'https://superinu-miniapp.vercel.app/api/frame/mint-superinu',
      });
    } catch (err) {
      console.error('Mint SuperInu failed:', err);
    }
  }
  ...
  return (
    <main className="min-h-screen bg-transparent flex flex-col items-center justify-start pt-6 transition-colors">
      ...
      <div className="flex gap-4 mb-2">
        {(['mint', 'editor', 'stats', 'about'] as TabKey[]).map((key) => (
          <button
            key={key}
      ...
      {activeTab === 'mint' && (
        <div className="m-4 p-6 bg-white/5 border border-green-700/50 rounded-2xl shadow-lg space-y-6 text-center w-full max-w-md">
          <h1 className="text-2xl font-extrabold text-green-300">🐶 Mint Your SuperInu</h1>
          <p className="opacity-90">
            Create your personalized SuperInu NFT based on your Farcaster profile.
          </p>

          <button
            onClick={handleMintSuperInu}
          ...
          {!canUseStickers && (
            <div className="mt-4 rounded-xl border border-yellow-400/40 bg-yellow-500/10 p-4">
              <p className="font-semibold text-yellow-300">
                You need to hold <span className="font-black">$SUPERINU</span> in at least one
                wallet to use stickers.
              </p>
              <div className="mt-3">
                <button
                  onClick={handleBuy}
                  className="px-5 py-2 rounded-lg bg-fuchsia-600 hover:bg-fuchsia-700"
                >
                  Buy Now
                </button>
              </div>
